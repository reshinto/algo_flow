...
std::vector<int> buildFailureTable(const std::string& pattern) {
    std::vector<int> failure(pattern.length(), 0); // @step:build-failure
    int prefixLen = 0; // @step:build-failure
    int tableIdx = 1; // @step:build-failure

    while (tableIdx < static_cast<int>(pattern.length())) {
...
int kmpSearch(const std::string& text, const std::string& pattern) {
    ...
    auto failure = buildFailureTable(pattern); // @step:initialize

    int textIdx = 0; // @step:initialize
    int patternIdx = 0; // @step:initialize

    while (textIdx < static_cast<int>(text.length())) {
        // @step:visit
        if (text[textIdx] == pattern[patternIdx]) {
            ...
            if (patternIdx == static_cast<int>(pattern.length())) {
