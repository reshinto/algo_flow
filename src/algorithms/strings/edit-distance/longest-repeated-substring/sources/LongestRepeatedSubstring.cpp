...
std::string longestRepeatedSubstring(const std::string& text) {
    int textLength = static_cast<int>(text.length()); // @step:initialize

    // Allocate (textLength+1) × (textLength+1) DP matrix
    std::vector<std::vector<int>> dp(textLength + 1, std::vector<int>(textLength + 1, 0)); // @step:initialize
    ...
    for (int rowIdx = 1; rowIdx <= textLength; rowIdx++) {
        for (int colIdx = 1; colIdx <= textLength; colIdx++) {
            ...
            char rowChar = text[rowIdx - 1]; // @step:compare
            char colChar = text[colIdx - 1]; // @step:compare
    ...
    return text.substr(longestEndIndex - longestLength, longestLength); // @step:complete
}
