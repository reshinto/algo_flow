...
comb_sort_module = importlib.import_module("comb-sort")
comb_sort = comb_sort_module.comb_sort


def test_sorts_unsorted_array():
    assert comb_sort([64, 34, 25, 12, 22, 11, 90]) == [11, 12, 22, 25, 34, 64, 90]
...
def test_handles_array_with_negative_numbers():
    assert comb_sort([3, -1, 0, -5, 2]) == [-5, -1, 0, 2, 3]


def test_does_not_mutate_original_array():
    original = [3, 1, 2]
    sorted_result = comb_sort(original)
