...
describe("combSort", () => {
  it("sorts an unsorted array", () => {
    expect(combSort([64, 34, 25, 12, 22, 11, 90])).toEqual([11, 12, 22, 25, 34, 64, 90]);
  });
  ...
  it("handles an array with duplicate values", () => {
    expect(combSort([3, 1, 4, 1, 5, 9, 2, 6, 5])).toEqual([1, 1, 2, 3, 4, 5, 5, 6, 9]);
  });
  ...
  it("handles an array with negative numbers", () => {
    expect(combSort([3, -1, 0, -5, 2])).toEqual([-5, -1, 0, 2, 3]);
  });

  it("does not mutate the original array", () => {
    const original = [3, 1, 2];
    const sorted = combSort(original);
