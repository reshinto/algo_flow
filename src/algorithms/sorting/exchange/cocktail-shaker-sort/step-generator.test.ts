...
describe("generateCocktailShakerSortSteps", () => {
  it("generates steps for a simple array", () => {
    const steps = generateCocktailShakerSortSteps([3, 1, 2]);
  ...
  it("includes compare and swap steps", () => {
    const steps = generateCocktailShakerSortSteps([3, 1]);
  ...
  it("handles a single element array", () => {
    const steps = generateCocktailShakerSortSteps([42]);
    expect(steps.length).toBeGreaterThan(0);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });

  it("correctly sorts via step execution — final state matches sorted input", () => {
    const steps = generateCocktailShakerSortSteps([5, 3, 8, 1, 9, 2]);
    const lastStep = steps[steps.length - 1]!;
    const visualState = lastStep.visualState as ArrayVisualState;
    const values = visualState.elements.map((el) => el.value);
    expect(values).toEqual([1, 2, 3, 5, 8, 9]);
  });
});
