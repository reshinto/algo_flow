...
describe("generateRadixSortMsdSteps", () => {
  it("generates steps for a simple array", () => {
    const steps = generateRadixSortMsdSteps([3, 1, 2]);
  ...
  it("includes compare steps for digit extraction", () => {
    const steps = generateRadixSortMsdSteps([3, 1, 2]);
    const stepTypes = steps.map((step) => step.type);
    expect(stepTypes).toContain("compare");
  });

  it("includes swap steps for collection phase", () => {
    const steps = generateRadixSortMsdSteps([3, 1, 2]);
    const stepTypes = steps.map((step) => step.type);
    expect(stepTypes).toContain("swap");
  });

  it("marks all elements sorted after final pass", () => {
    const steps = generateRadixSortMsdSteps([3, 1, 2]);
  ...
  it("handles a single element array", () => {
    const steps = generateRadixSortMsdSteps([42]);
    expect(steps.length).toBeGreaterThan(0);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });

  it("handles an empty array", () => {
    const steps = generateRadixSortMsdSteps([]);
    expect(steps.length).toBeGreaterThan(0);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });
  ...
  it("final visual state values match sorted order for default E2E input", () => {
    ...
    const steps = generateRadixSortMsdSteps(input);
    const lastStep = steps[steps.length - 1]!;
    const visualState = lastStep.visualState as ArrayVisualState;
