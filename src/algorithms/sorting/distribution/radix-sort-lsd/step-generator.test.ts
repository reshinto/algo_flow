...
describe("generateRadixSortLsdSteps", () => {
  it("generates steps for a simple array", () => {
    const steps = generateRadixSortLsdSteps([3, 1, 2]);
  ...
  it("includes compare steps for digit extraction phase", () => {
    const steps = generateRadixSortLsdSteps([3, 1, 2]);
  ...
  it("handles a single element array", () => {
    const steps = generateRadixSortLsdSteps([42]);
    expect(steps.length).toBeGreaterThan(0);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });

  it("handles an empty array", () => {
    const steps = generateRadixSortLsdSteps([]);
    expect(steps.length).toBeGreaterThan(0);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });
});
