...
pigeonhole_sort_module = importlib.import_module("pigeonhole-sort")
pigeonhole_sort = pigeonhole_sort_module.pigeonhole_sort


def test_sorts_unsorted_array():
    assert pigeonhole_sort([64, 34, 25, 12, 22, 11, 90]) == [11, 12, 22, 25, 34, 64, 90]
...
def test_handles_array_with_duplicate_values():
    assert pigeonhole_sort([3, 1, 4, 1, 5, 9, 2, 6, 5]) == [1, 1, 2, 3, 4, 5, 5, 6, 9]
...
def test_handles_negative_numbers_using_offset():
    assert pigeonhole_sort([3, -1, 0, -5, 2]) == [-5, -1, 0, 2, 3]


def test_handles_array_where_all_elements_are_the_same():
    assert pigeonhole_sort([7, 7, 7, 7]) == [7, 7, 7, 7]


def test_does_not_mutate_original_array():
    original = [3, 1, 2]
    sorted_result = pigeonhole_sort(original)
