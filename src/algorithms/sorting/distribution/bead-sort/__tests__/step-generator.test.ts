...
describe("generateBeadSortSteps", () => {
  it("generates steps for a simple array", () => {
    const steps = generateBeadSortSteps([3, 1, 2]);
  ...
  it("includes compare and swap steps for gravity drops", () => {
    const steps = generateBeadSortSteps([3, 1, 2]);
  ...
  it("handles a single element array", () => {
    const steps = generateBeadSortSteps([42]);
    expect(steps.length).toBeGreaterThan(0);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });

  it("handles an empty array", () => {
    const steps = generateBeadSortSteps([]);
    expect(steps.length).toBeGreaterThan(0);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });

  it("correctly sorts the default input", () => {
    const steps = generateBeadSortSteps([64, 34, 25, 12, 22, 11, 90]);
    const lastStep = steps[steps.length - 1]!;
    const visualState = lastStep.visualState as ArrayVisualState;
  ...
  it("generates steps with swap descriptions for gravity drops", () => {
    const steps = generateBeadSortSteps([3, 1, 2]);
    const swapStep = steps.find((step) => step.type === "swap");
    expect(swapStep).toBeDefined();
    expect(swapStep!.description).toContain("Gravity");
  });
});
