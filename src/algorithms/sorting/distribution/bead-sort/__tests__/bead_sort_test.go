...
func TestSortsUnsortedArray(t *testing.T) {
	result := beadSort([]int{64, 34, 25, 12, 22, 11, 90})
...
func TestHandlesArrayWithDuplicateValues(t *testing.T) {
	result := beadSort([]int{3, 1, 4, 1, 5, 9, 2, 6, 5})
...
func TestHandlesArrayWithAllZeros(t *testing.T) {
	result := beadSort([]int{0, 0, 0})
	expected := []int{0, 0, 0}
	if !reflect.DeepEqual(result, expected) {
		t.Errorf("expected %v, got %v", expected, result)
	}
...
func TestHandlesNegativeNumbersByOffsetting(t *testing.T) {
	result := beadSort([]int{3, -1, 0, -5, 2})
