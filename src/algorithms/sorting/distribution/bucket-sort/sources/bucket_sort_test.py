...
bucket_sort_module = importlib.import_module("bucket-sort")
bucket_sort = bucket_sort_module.bucket_sort
...
def test_handles_array_with_duplicate_values():
    assert bucket_sort([3, 1, 4, 1, 5, 9, 2, 6, 5]) == [1, 1, 2, 3, 4, 5, 5, 6, 9]
...
def test_handles_negative_numbers():
    assert bucket_sort([3, -1, 0, -5, 2]) == [-5, -1, 0, 2, 3]


def test_handles_array_where_all_elements_are_the_same():
    assert bucket_sort([7, 7, 7, 7]) == [7, 7, 7, 7]


def test_does_not_mutate_original_array():
    original = [3, 1, 2]
    sorted_result = bucket_sort(original)
