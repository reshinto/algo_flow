...
describe("generateBucketSortSteps", () => {
  it("generates steps for a simple array", () => {
    const steps = generateBucketSortSteps([3, 1, 2]);
  ...
  it("includes compare steps for distribution and bucket sorting", () => {
    const steps = generateBucketSortSteps([3, 1, 2]);
    const stepTypes = steps.map((step) => step.type);
    expect(stepTypes).toContain("compare");
  });

  it("includes swap steps for collection and insertion sort", () => {
    const steps = generateBucketSortSteps([3, 1, 2]);
    const stepTypes = steps.map((step) => step.type);
  ...
  it("handles a single element array", () => {
    const steps = generateBucketSortSteps([42]);
    expect(steps.length).toBeGreaterThan(0);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });

  it("handles an empty array", () => {
    const steps = generateBucketSortSteps([]);
    expect(steps.length).toBeGreaterThan(0);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });
});
