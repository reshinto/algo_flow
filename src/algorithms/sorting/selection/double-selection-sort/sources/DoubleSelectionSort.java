public class DoubleSelectionSort {
    public static int[] doubleSelectionSort(int[] inputArray) { // @step:initialize
        ...
        while (leftBound < rightBound) {
            ...
            for (int scanIndex = leftBound + 1; scanIndex <= rightBound; scanIndex++) { // @step:compare
                if (sortedArray[scanIndex] < sortedArray[minimumIndex]) { // @step:compare
                    minimumIndex = scanIndex; // @step:compare
                }
                if (sortedArray[scanIndex] > sortedArray[maximumIndex]) { // @step:compare
                    maximumIndex = scanIndex; // @step:compare
                }
            }

            // Swap minimum to left bound
            if (minimumIndex != leftBound) { // @step:swap
                int temporaryMin = sortedArray[leftBound]; // @step:swap
                ...
                if (maximumIndex == leftBound) { // @step:swap
                    maximumIndex = minimumIndex; // @step:swap
                }
            }

            // Swap maximum to right bound
            if (maximumIndex != rightBound) { // @step:swap
                int temporaryMax = sortedArray[rightBound]; // @step:swap
