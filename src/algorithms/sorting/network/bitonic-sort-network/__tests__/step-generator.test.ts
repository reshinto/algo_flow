...
describe("generateBitonicSortNetworkSteps", () => {
  it("generates steps for a simple array", () => {
    const steps = generateBitonicSortNetworkSteps([4, 2, 3, 1]);
  ...
  it("handles a single element array", () => {
    const steps = generateBitonicSortNetworkSteps([42]);
    expect(steps.length).toBeGreaterThan(0);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });

  it("handles power-of-2 sized arrays", () => {
    const steps = generateBitonicSortNetworkSteps([8, 6, 4, 2, 7, 5, 3, 1]);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });
});
