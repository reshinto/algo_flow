...
describe("generateQuickSort3WaySteps", () => {
  it("generates steps for a simple array", () => {
    const steps = generateQuickSort3WaySteps([3, 1, 2]);
  ...
  it("handles array with all duplicate values efficiently", () => {
    const steps = generateQuickSort3WaySteps([5, 5, 5, 5]);
    const lastStep = steps[steps.length - 1]!;
    expect(lastStep.type).toBe("complete");
    // All duplicates should be handled in very few steps
  ...
  it("includes highlighted lines for each step", () => {
    const steps = generateQuickSort3WaySteps([3, 1]);
  ...
  it("handles a single element array", () => {
    const steps = generateQuickSort3WaySteps([42]);
    expect(steps.length).toBeGreaterThan(0);
