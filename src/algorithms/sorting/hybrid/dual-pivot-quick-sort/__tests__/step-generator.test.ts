...
describe("generateDualPivotQuickSortSteps", () => {
  it("generates steps for a simple array", () => {
    const steps = generateDualPivotQuickSortSteps([3, 1, 2]);
  ...
  it("includes compare and swap steps", () => {
    const steps = generateDualPivotQuickSortSteps([4, 2, 6, 1, 5]);
    const stepTypes = steps.map((step) => step.type);
    expect(stepTypes).toContain("compare");
    expect(stepTypes).toContain("swap");
  });

  it("marks elements as sorted after each partition", () => {
    const steps = generateDualPivotQuickSortSteps([4, 2, 6, 1, 5]);
  ...
  it("includes highlighted lines for each step", () => {
    const steps = generateDualPivotQuickSortSteps([3, 1, 4, 2]);
  ...
  it("handles a single element array", () => {
    const steps = generateDualPivotQuickSortSteps([42]);
    expect(steps.length).toBeGreaterThan(0);
