...
describe("generateBitonicSortSteps", () => {
  ...
  it("correctly sorts a power-of-2 sized array", () => {
    const steps = generateBitonicSortSteps([4, 2, 6, 1, 5, 3, 8, 7]);
    const lastStep = steps[steps.length - 1]!;
    expect(lastStep.type).toBe("complete");
  });
  ...
  it("handles a single element array", () => {
    const steps = generateBitonicSortSteps([42]);
    expect(steps.length).toBeGreaterThan(0);
