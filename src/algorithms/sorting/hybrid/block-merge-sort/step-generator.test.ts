...
describe("generateBlockMergeSortSteps", () => {
  it("generates steps for a simple array", () => {
    const steps = generateBlockMergeSortSteps([3, 1, 2]);
  ...
  it("marks elements as sorted after merge passes", () => {
    const steps = generateBlockMergeSortSteps([3, 1, 2]);
  ...
  it("handles a single element array", () => {
    const steps = generateBlockMergeSortSteps([42]);
    expect(steps.length).toBeGreaterThan(0);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });

  it("handles an already sorted array efficiently", () => {
    const steps = generateBlockMergeSortSteps([1, 2, 3, 4, 5]);
    // Sorted input is one natural run — minimal steps
    expect(steps.length).toBeGreaterThan(0);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });
});
