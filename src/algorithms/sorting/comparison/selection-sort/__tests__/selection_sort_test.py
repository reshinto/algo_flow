...
selection_sort_module = importlib.import_module("selection-sort")
selection_sort = selection_sort_module.selection_sort
...
def test_handles_array_with_negative_numbers():
    assert selection_sort([3, -1, 0, -5, 2]) == [-5, -1, 0, 2, 3]


def test_does_not_mutate_original_array():
    original = [3, 1, 2]
    sorted_result = selection_sort(original)
