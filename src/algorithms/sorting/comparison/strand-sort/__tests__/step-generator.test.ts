...
describe("generateStrandSortSteps", () => {
  it("generates steps for a simple array", () => {
    const steps = generateStrandSortSteps([3, 1, 2]);
  ...
  it("includes compare steps during strand extraction", () => {
    const steps = generateStrandSortSteps([3, 1, 2]);
  ...
  it("handles a single element array", () => {
    const steps = generateStrandSortSteps([42]);
    expect(steps.length).toBeGreaterThan(0);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });

  it("generates fewer compare steps for sorted input (only one strand)", () => {
    const sortedSteps = generateStrandSortSteps([1, 2, 3, 4]);
    const unsortedSteps = generateStrandSortSteps([4, 3, 2, 1]);
    // Sorted input should need fewer steps — one strand covers everything
    expect(sortedSteps.length).toBeLessThan(unsortedSteps.length);
  });

  it("produces correct sorted values in final state", () => {
    const steps = generateStrandSortSteps([5, 3, 1, 4, 2]);
    const lastStep = steps[steps.length - 1]!;
    const visualState = lastStep.visualState as ArrayVisualState;
    expect(visualState.elements.map((el) => el.value)).toEqual([1, 2, 3, 4, 5]);
  });
});
