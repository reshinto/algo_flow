...
describe("generatePatienceSortSteps", () => {
  it("generates steps for a simple array", () => {
    const steps = generatePatienceSortSteps([3, 1, 2]);
  ...
  it("includes compare and swap steps", () => {
    const steps = generatePatienceSortSteps([3, 1, 4, 2]);
  ...
  it("handles a single element array", () => {
    const steps = generatePatienceSortSteps([42]);
    expect(steps.length).toBeGreaterThan(0);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });

  it("handles an empty array", () => {
    const steps = generatePatienceSortSteps([]);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });

  it("final sorted order is correct", () => {
    const steps = generatePatienceSortSteps([3, 1, 4, 1, 5]);
    const lastVisual = steps[steps.length - 1]!.visualState as ArrayVisualState;
    expect(lastVisual.elements.map((el) => el.value)).toEqual([1, 1, 3, 4, 5]);
  });
});
