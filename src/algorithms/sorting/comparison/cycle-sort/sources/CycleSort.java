public class CycleSort {
    public static int[] cycleSort(int[] inputArray) { // @step:initialize
        int[] sortedArray = inputArray.clone(); // @step:initialize
        int arrayLength = sortedArray.length; // @step:initialize

        for (int cycleStart = 0; cycleStart < arrayLength - 1; cycleStart++) { // @step:count-position
            ...
            for (int scanIndex = cycleStart + 1; scanIndex < arrayLength; scanIndex++) { // @step:compare
                if (sortedArray[scanIndex] < currentValue) { // @step:compare
                    correctPosition++; // @step:count-position
                }
            }

            // If the item is already in the correct position, skip this cycle
            if (correctPosition == cycleStart) { // @step:count-position
                continue; // @step:count-position
            }

            // Skip over duplicates to find the unique insertion point
            while (currentValue == sortedArray[correctPosition]) { // @step:count-position
                correctPosition++; // @step:count-position
            }
            ...
            while (correctPosition != cycleStart) { // @step:count-position
                ...
                if (currentValue != sortedArray[correctPosition]) { // @step:swap
