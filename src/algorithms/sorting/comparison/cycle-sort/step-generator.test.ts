...
describe("generateCycleSortSteps", () => {
  it("generates steps for a simple array", () => {
    const steps = generateCycleSortSteps([3, 1, 2]);
  ...
  it("handles a single element array", () => {
    const steps = generateCycleSortSteps([42]);
    expect(steps.length).toBeGreaterThan(0);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });

  it("handles an already-sorted array", () => {
    const steps = generateCycleSortSteps([1, 2, 3]);
    expect(steps[steps.length - 1]!.type).toBe("complete");
    const lastVisual = steps[steps.length - 1]!.visualState as ArrayVisualState;
    expect(lastVisual.elements.map((el) => el.value)).toEqual([1, 2, 3]);
  });

  it("final sorted order is correct", () => {
    const steps = generateCycleSortSteps([3, 1, 5, 2, 4]);
    const lastVisual = steps[steps.length - 1]!.visualState as ArrayVisualState;
    expect(lastVisual.elements.map((el) => el.value)).toEqual([1, 2, 3, 4, 5]);
  });
});
