...
describe("generateIntroSortSteps", () => {
  it("generates steps for a simple array", () => {
    const steps = generateIntroSortSteps([3, 1, 2]);
  ...
  it("includes compare and swap steps", () => {
    const steps = generateIntroSortSteps([3, 1, 4, 2]);
  ...
  it("handles a single element array", () => {
    const steps = generateIntroSortSteps([42]);
    expect(steps.length).toBeGreaterThan(0);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });

  it("handles an already-sorted array", () => {
    const steps = generateIntroSortSteps([1, 2, 3]);
    expect(steps[steps.length - 1]!.type).toBe("complete");
