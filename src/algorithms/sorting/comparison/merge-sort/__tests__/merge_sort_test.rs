...
mod tests {
    ...
    fn sorts_unsorted_array() {
        assert_eq!(merge_sort(&[64, 34, 25, 12, 22, 11, 90]), vec![11, 12, 22, 25, 34, 64, 90]);
    }
    ...
    fn handles_array_with_duplicate_values() {
        assert_eq!(merge_sort(&[3, 1, 4, 1, 5, 9, 2, 6, 5]), vec![1, 1, 2, 3, 4, 5, 5, 6, 9]);
    }
    ...
    fn handles_array_with_negative_numbers() {
        assert_eq!(merge_sort(&[3, -1, 0, -5, 2]), vec![-5, -1, 0, 2, 3]);
    }

    #[test]
    fn does_not_mutate_original_array() {
        let original = vec![3, 1, 2];
        let sorted = merge_sort(&original);
