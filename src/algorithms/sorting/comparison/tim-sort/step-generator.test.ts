...
describe("generateTimSortSteps", () => {
  it("generates steps for a simple array", () => {
    const steps = generateTimSortSteps([3, 1, 2]);
  ...
  it("includes compare and swap steps", () => {
    const steps = generateTimSortSteps([3, 1, 4, 2]);
  ...
  it("handles a single element array", () => {
    const steps = generateTimSortSteps([42]);
    expect(steps.length).toBeGreaterThan(0);
    expect(steps[steps.length - 1]!.type).toBe("complete");
  });

  it("handles an already-sorted array", () => {
    const steps = generateTimSortSteps([1, 2, 3, 4]);
    expect(steps[steps.length - 1]!.type).toBe("complete");
    const lastVisual = steps[steps.length - 1]!.visualState as ArrayVisualState;
    expect(lastVisual.elements.map((el) => el.value)).toEqual([1, 2, 3, 4]);
  });
});
