...
insertion_sort_module = importlib.import_module("insertion-sort")
insertion_sort = insertion_sort_module.insertion_sort
...
def test_handles_array_with_negative_numbers():
    assert insertion_sort([3, -1, 0, -5, 2]) == [-5, -1, 0, 2, 3]


def test_does_not_mutate_original_array():
    original = [3, 1, 2]
    sorted_result = insertion_sort(original)
