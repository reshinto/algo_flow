...
func TestSortsUnsortedArray(t *testing.T) {
	result := cubeSort([]int{64, 34, 25, 12, 22, 11, 90})
...
func TestHandlesArrayWithDuplicateValues(t *testing.T) {
	result := cubeSort([]int{3, 1, 4, 1, 5, 9, 2, 6, 5})
...
func TestHandlesArrayOfSize27PerfectCube(t *testing.T) {
	input := make([]int, 27)
	expected := make([]int, 27)
	for idx := 0; idx < 27; idx++ {
		input[idx] = 27 - idx
		expected[idx] = idx + 1
	}
	result := cubeSort(input)
	if !reflect.DeepEqual(result, expected) {
		t.Errorf("expected %v, got %v", expected, result)
	}
}
