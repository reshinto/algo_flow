...
mod tests {
    ...
    fn traverses_linear_graph_in_order() {
        ...
        assert_eq!(breadth_first_search(&adj, "A"), to_strings(&["A","B","C","D"]));
    }

    #[test]
    fn traverses_tree_graph_level_by_level() {
        let adj = make_adj(&[
            ("A", &["B","C"]), ("B", &["D","E"]), ("C", &["F"]),
            ("D", &[]), ("E", &[]), ("F", &[]),
        ]);
        assert_eq!(breadth_first_search(&adj, "A"), to_strings(&["A","B","C","D","E","F"]));
    }

    #[test]
    fn handles_disconnected_graph_visiting_only_reachable_nodes() {
        let adj = make_adj(&[("A", &["B"]), ("B", &[]), ("C", &["D"]), ("D", &[])]);
        let result = breadth_first_search(&adj, "A");
    ...
    fn handles_single_node_graph() {
        let adj = make_adj(&[("A", &[])]);
        assert_eq!(breadth_first_search(&adj, "A"), to_strings(&["A"]));
    }
    ...
    #[test]
    fn traverses_fully_connected_graph() {
        let adj = make_adj(&[
            ("A", &["B","C","D"]), ("B", &["A","C","D"]),
            ("C", &["A","B","D"]), ("D", &["A","B","C"]),
        ]);
