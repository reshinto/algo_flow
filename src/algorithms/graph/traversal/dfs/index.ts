/* DFS algorithm registration — assembles the definition (meta, execute, steps,
...
import { dfsEducational } from "./educational";
...
const dfsDefinition: AlgorithmDefinition<DfsInput> = {
  meta: {
    id: ALGORITHM_ID.DFS!,
    ...
    technique: "traversal",
    description:
      "A graph traversal algorithm that explores as far as possible along each branch before backtracking to explore other paths",
    timeComplexity: {
      best: "O(V+E)",
      average: "O(V+E)",
      worst: "O(V+E)",
...
registry.register(dfsDefinition);
