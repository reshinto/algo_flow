...
int main() {
    // Test 1: finds three SCCs in default 8-node graph
    {
        ...
        TarjanSCC ts;
        auto result = ts.tarjanSCC(adjacencyList, {"A", "B", "C", "D", "E", "F", "G", "H"});
    ...
    // Test 2: finds single SCC for fully cyclic graph
    {
        ...
        TarjanSCC ts;
        auto result = ts.tarjanSCC(adjacencyList, {"A", "B", "C"});
    ...
    {
        unordered_map<string, vector<string>> adjacencyList = {{"A", {"B"}}, {"B", {"C"}}, {"C", {}}};
        TarjanSCC ts;
        auto result = ts.tarjanSCC(adjacencyList, {"A", "B", "C"});
        assert(result.size() == 3);
    ...
    // Test 4: handles single node with no edges
    {
        ...
        TarjanSCC ts;
        auto result = ts.tarjanSCC(adjacencyList, {"A"});
        assert(result.size() == 1);
        assert((result[0] == vector<string>{"A"}));
    }

    // Test 5: handles disconnected directed graph
    {
        unordered_map<string, vector<string>> adjacencyList = {
            {"A", {"B"}}, {"B", {"A"}}, {"C", {"D"}}, {"D", {"C"}},
        };
        TarjanSCC ts;
        auto result = ts.tarjanSCC(adjacencyList, {"A", "B", "C", "D"});
    ...
    // Test 6: assigns every node to exactly one SCC
    {
        ...
        TarjanSCC ts;
        auto result = ts.tarjanSCC(adjacencyList, {"A", "B", "C", "D", "E"});
    ...
    // Test 7: correctly handles self-loops as single-node SCCs
    {
        unordered_map<string, vector<string>> adjacencyList = {{"A", {"A"}}, {"B", {}}};
        TarjanSCC ts;
        auto result = ts.tarjanSCC(adjacencyList, {"A", "B"});
        assert(result.size() == 2);
        vector<set<string>> compSets;
        for (auto& comp : result) compSets.push_back(set<string>(comp.begin(), comp.end()));
        assert((find(compSets.begin(), compSets.end(), set<string>{"A"}) != compSets.end()));
        assert((find(compSets.begin(), compSets.end(), set<string>{"B"}) != compSets.end()));
    }
