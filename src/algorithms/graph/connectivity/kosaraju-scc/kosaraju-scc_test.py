...
module = importlib.import_module("kosaraju-scc")
kosaraju_scc = module.kosaraju_scc
...
def test_finds_three_sccs_in_default_8_node_graph():
    ...
    result = kosaraju_scc(adjacency_list, node_ids)
    assert len(result) == 3
...
def test_finds_single_scc_for_fully_cyclic_graph():
    ...
    result = kosaraju_scc(adjacency_list, ["A", "B", "C"])
    assert len(result) == 1
...
def test_handles_single_node_with_no_edges():
    result = kosaraju_scc({"A": []}, ["A"])
    assert len(result) == 1
    assert result[0] == ["A"]


def test_handles_disconnected_directed_graph_with_two_mutual_pairs():
    adjacency_list = {"A": ["B"], "B": ["A"], "C": ["D"], "D": ["C"]}
    result = kosaraju_scc(adjacency_list, ["A", "B", "C", "D"])
    assert len(result) == 2
    comp_sets = [frozenset(comp) for comp in result]
...
def test_assigns_every_node_to_exactly_one_scc_with_no_duplicates():
    adjacency_list = {
...
def test_produces_same_scc_groupings_for_known_graph():
    adjacency_list = {
        "A": ["B"], "B": ["C"], "C": ["A"], "D": ["E"], "E": ["D"],
    }
    result = kosaraju_scc(adjacency_list, ["A", "B", "C", "D", "E"])
    assert len(result) == 2
    comp_sets = [frozenset(comp) for comp in result]
    assert frozenset(["A", "B", "C"]) in comp_sets
    assert frozenset(["D", "E"]) in comp_sets
...
if __name__ == "__main__":
    ...
    test_handles_disconnected_directed_graph_with_two_mutual_pairs()
    test_assigns_every_node_to_exactly_one_scc_with_no_duplicates()
    test_produces_same_scc_groupings_for_known_graph()
