...
describe("generateKosarajuSccSteps", () => {
  ...
  it("includes push-stack steps from first pass", () => {
    const input: KosarajuSccInput = {
  ...
  it("includes pop-stack steps from second pass", () => {
    const input: KosarajuSccInput = {
      adjacencyList: { A: ["B"], B: ["A"] },
      nodeIds: ["A", "B"],
      nodes: makeNodes(["A", "B"]),
  ...
  it("produces correct number of components for default 8-node graph", () => {
    const input: KosarajuSccInput = {
      adjacencyList: {
        A: ["B"],
        B: ["C"],
