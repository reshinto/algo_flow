...
int main() {
    auto makeEdge = [](const string& src, const string& tgt, int w) {
        return Kruskals::WeightedEdge{src, tgt, w};
    };
    auto totalWeight = [](const vector<Kruskals::WeightedEdge>& edges) {
        int sum = 0;
    ...
    // Test 1: 6-node MST
    {
        vector<Kruskals::WeightedEdge> edges = {
    ...
    // Test 2: V-1 edges
    {
        vector<Kruskals::WeightedEdge> edges = {makeEdge("A","B",3), makeEdge("A","C",1), makeEdge("B","C",2)};
    ...
    {
        vector<Kruskals::WeightedEdge> edges = {makeEdge("A","B",10), makeEdge("B","C",1), makeEdge("A","C",5)};
        auto result = Kruskals::kruskalsAlgorithm(edges, {"A","B","C"});
        assert(result.size() == 2);
        vector<int> weights;
    ...
    {
        vector<Kruskals::WeightedEdge> edges = {makeEdge("A","B",1), makeEdge("B","C",2), makeEdge("A","C",3)};
        auto result = Kruskals::kruskalsAlgorithm(edges, {"A","B","C"});
        assert(result.size() == 2);
    ...
    {
        vector<Kruskals::WeightedEdge> edges = {makeEdge("A","B",7)};
    ...
    // Test 6: linear chain
    {
        vector<Kruskals::WeightedEdge> edges = {makeEdge("A","B",2), makeEdge("B","C",4), makeEdge("C","D",1)};
