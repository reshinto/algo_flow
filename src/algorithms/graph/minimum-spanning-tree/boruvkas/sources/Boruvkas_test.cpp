...
int main() {
    auto makeEdge = [](const string& src, const string& tgt, int w) {
        return Boruvkas::WeightedEdge{src, tgt, w};
    };
    auto totalWeight = [](const vector<Boruvkas::WeightedEdge>& edges) {
        int sum = 0;
    ...
    // Test 1: 6-node MST
    {
        vector<Boruvkas::WeightedEdge> edges = {
    ...
    // Test 2: V-1 edges
    {
        vector<Boruvkas::WeightedEdge> edges = {makeEdge("A","B",3), makeEdge("A","C",1), makeEdge("B","C",2)};
    ...
    {
        vector<Boruvkas::WeightedEdge> edges = {makeEdge("A","B",1), makeEdge("B","C",5), makeEdge("A","C",3)};
        auto result = Boruvkas::boruvkasAlgorithm(edges, {"A","B","C"});
        assert(result.size() == 2);
        vector<int> weights;
    ...
    {
        vector<Boruvkas::WeightedEdge> edges = {makeEdge("A","B",2), makeEdge("B","C",3), makeEdge("A","C",10)};
        auto result = Boruvkas::boruvkasAlgorithm(edges, {"A","B","C"});
    ...
    {
        vector<Boruvkas::WeightedEdge> edges = {makeEdge("A","B",6)};
    ...
    // Test 6: linear chain
    {
        vector<Boruvkas::WeightedEdge> edges = {makeEdge("A","B",1), makeEdge("B","C",2), makeEdge("C","D",3)};
