...
def test_computes_max_flow_across_two_parallel_paths():
    ...
    assert edmonds_karp(graph, "S", "T") == 10
...
def test_computes_max_flow_for_default_6_node_network():
    ...
    assert edmonds_karp(graph, "S", "T") == 17


def test_returns_zero_when_no_path_from_source_to_sink():
    graph = {"S": [{"target": "A", "capacity": 10}], "A": [], "T": []}
    assert edmonds_karp(graph, "S", "T") == 0


def test_produces_same_max_flow_as_ford_fulkerson():
    graph = {
    ...
    assert edmonds_karp(graph, "S", "T") == 6


def test_handles_graph_where_source_has_no_outgoing_edges():
    graph = {"S": [], "T": []}
    assert edmonds_karp(graph, "S", "T") == 0


def test_handles_three_path_diamond_graph_correctly():
    graph = {
        "S": [{"target": "A", "capacity": 10}, {"target": "B", "capacity": 10}],
        "A": [{"target": "T", "capacity": 10}],
        "B": [{"target": "T", "capacity": 10}],
        "T": [],
    }
    assert edmonds_karp(graph, "S", "T") == 20
...
if __name__ == "__main__":
    ...
    test_produces_same_max_flow_as_ford_fulkerson()
    test_handles_graph_where_source_has_no_outgoing_edges()
    test_handles_three_path_diamond_graph_correctly()
