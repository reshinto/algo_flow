public class FibonacciSearch {
    public static int fibonacciSearch(int[] sortedArray, int targetValue) { // @step:initialize
        ...
        while (fibM < arrayLength) { // @step:initialize
        ...
        while (fibM > 1) {
            int compareIndex = Math.min(offset + fibM2, arrayLength - 1); // @step:compare
            int compareValue = sortedArray[compareIndex]; // @step:compare

            if (compareValue < targetValue) { // @step:eliminate
            ...
            } else if (compareValue > targetValue) { // @step:eliminate
        ...
        if (fibM1 == 1 && offset + 1 < arrayLength && sortedArray[offset + 1] == targetValue) { // @step:compare,found
