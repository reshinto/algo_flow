...
describe("generateRecursiveBinarySearchSteps", () => {
  ...
  it("simulates recursion correctly — multiple eliminate steps for deep search", () => {
    const steps = generateRecursiveBinarySearchSteps({
    ...
    expect(eliminateSteps.length).toBeGreaterThan(1);
  });
});
