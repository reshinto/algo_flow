...
build_min_heap = importlib.import_module("build-min-heap").build_min_heap
...
def test_valid_min_heap():
    result = build_min_heap([9, 5, 7, 1, 3, 8, 2, 6, 4])
...
def test_preserves_all_elements():
    ...
    result = build_min_heap(input_arr)
    assert sorted(result) == sorted(input_arr)


def test_already_valid_min_heap():
    result = build_min_heap([1, 3, 2, 7, 5, 8, 4])
    assert is_min_heap(result)
    assert result[0] == 1


def test_reverse_sorted():
    result = build_min_heap([7, 6, 5, 4, 3, 2, 1])
    assert is_min_heap(result)
    assert result[0] == 1


def test_single_element():
    result = build_min_heap([42])
    assert result == [42]


def test_two_elements():
    result = build_min_heap([5, 2])
    assert result[0] == 2
    assert is_min_heap(result)
