include!("sources/build-heap-top-down.rs");
...
mod tests {
    ...
    fn test_valid_min_heap() {
        let result = build_heap_top_down(&[9,5,7,1,3,8,2,6,4]);
    ...
    fn test_preserves_elements() {
        let input = vec![9,5,7,1,3,8,2,6,4];
        let mut result = build_heap_top_down(&input);
        result.sort();
    ...
    fn test_already_sorted() {
        let result = build_heap_top_down(&[1,2,3,4,5,6,7]);
        assert!(is_min_heap(&result));
        assert_eq!(result[0], 1);
    }
    ...
    fn test_reverse_sorted() {
        let result = build_heap_top_down(&[7,6,5,4,3,2,1]);
        assert!(is_min_heap(&result));
        assert_eq!(result[0], 1);
    }
    ...
    fn test_single_element() {
        assert_eq!(build_heap_top_down(&[42]), vec![42]);
    }

    #[test]
    fn test_two_elements() {
        let result = build_heap_top_down(&[5,2]);
