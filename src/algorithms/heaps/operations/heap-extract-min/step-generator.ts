/** Step generator for Heap Extract Min — produces ExecutionStep[] using HeapTracker. */
...
const HEAP_LINE_MAP = buildLineMapFromSources(ALGORITHM_ID.HEAP_EXTRACT_MIN!);

export interface HeapExtractMinInput {
  array: number[];
}

export function generateHeapExtractMinSteps(input: HeapExtractMinInput): ExecutionStep[] {
  const { array } = input;
  const values = [...array];
  ...
  // Remove the last node (the extracted minimum)
