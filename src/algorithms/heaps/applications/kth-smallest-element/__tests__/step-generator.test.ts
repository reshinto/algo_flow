...
describe("generateKthSmallestElementSteps", () => {
  ...
  it("final heap has exactly k nodes", () => {
    const kValue = 3;
    const steps = generateKthSmallestElementSteps({
  ...
  it("final complete step variables include the correct result", () => {
    const steps = generateKthSmallestElementSteps({ array: [7, 10, 4, 3, 20, 15, 8], kValue: 3 });
    const lastStep = steps[steps.length - 1]!;
    expect((lastStep.variables as { result: number }).result).toBe(7);
  });

  it("handles k = 1 (finds the minimum)", () => {
    const steps = generateKthSmallestElementSteps({ array: [7, 10, 4, 3], kValue: 1 });
    expect(steps[0]?.type).toBe("initialize");
    expect(steps[steps.length - 1]?.type).toBe("complete");
    const lastStep = steps[steps.length - 1]!;
    expect((lastStep.variables as { result: number }).result).toBe(3);
  });

  it("handles a single-element array", () => {
    const steps = generateKthSmallestElementSteps({ array: [7], kValue: 1 });
    expect(steps[0]?.type).toBe("initialize");
    expect(steps[steps.length - 1]?.type).toBe("complete");
  });
});
