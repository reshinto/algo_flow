...
def max_sum_subarray(input_array: list[int], window_size: int) -> dict:
    if len(input_array) == 0 or window_size <= 0 or window_size > len(input_array):  # @step:initialize
        return {"max_sum": 0, "window_start_index": 0}  # @step:initialize
    ...
    for right_index in range(window_size, len(input_array)):
        ...
        if current_sum > max_sum:  # @step:compare
            max_sum = current_sum  # @step:compare
            window_start_index = right_index - window_size + 1  # @step:compare

    return {"max_sum": max_sum, "window_start_index": window_start_index}  # @step:complete
