...
describe("generateSlidingWindowMinSumSteps", () => {
  ...
  it("includes shrink and expand steps for each slide", () => {
    const steps = generateSlidingWindowMinSumSteps({
      inputArray: [4, 2, 1, 7, 8, 1],
      windowSize: 3,
    });
  ...
  it("complete step variables contain minSum", () => {
    const steps = generateSlidingWindowMinSumSteps({
      inputArray: [4, 2, 1, 7, 8, 1],
      windowSize: 3,
    });
