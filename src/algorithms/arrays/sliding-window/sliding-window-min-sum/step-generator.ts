/** Step generator for Sliding Window (Min Sum) — produces ExecutionStep[] using ArrayTracker. */
...
const SLIDING_WINDOW_MIN_SUM_LINE_MAP = buildLineMapFromSources(
  ALGORITHM_ID.SLIDING_WINDOW_MIN_SUM!,
);
...
export function generateSlidingWindowMinSumSteps(input: SlidingWindowMinSumInput): ExecutionStep[] {
  const { inputArray, windowSize } = input;

  const tracker = new ArrayTracker([...inputArray], SLIDING_WINDOW_MIN_SUM_LINE_MAP);
  ...
  for (let rightIndex = windowSize; rightIndex < inputArray.length; rightIndex++) {
    ...
    tracker.visit(
      rightIndex,
      {
        ...
        minSum,
        windowStartIndex,
        comparison: `${currentSum} ${currentSum < minSum ? "<" : ">="} ${minSum}`,
      },
      `Compare current sum ${currentSum} with min sum ${minSum}`,
    );
  }
