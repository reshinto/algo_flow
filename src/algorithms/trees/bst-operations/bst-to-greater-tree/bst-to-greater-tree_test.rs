include!("sources/bst-to-greater-tree.rs");
...
mod tests {
    ...
    fn test_transforms_3_node_bst() {
        ...
        let result = bst_to_greater_tree(tree).unwrap();
    ...
    fn test_single_node() {
        let single = leaf(5);
        let result = bst_to_greater_tree(single).unwrap();
        assert_eq!(result.borrow().value, 5);
    }

    #[test]
    fn test_null_tree() {
        assert!(bst_to_greater_tree(None).is_none());
    }
}
