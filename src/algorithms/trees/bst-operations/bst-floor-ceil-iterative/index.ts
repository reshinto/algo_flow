...
import { bstFloorCeilIterativeEducational } from "./educational";
...
function executeBstFloorCeilIterative(input: BstFloorCeilIterativeInput): {
  floor: number | null;
  ceil: number | null;
} {
  ...
  return bstFloorCeilIterative(root, input.targetValue) as {
    floor: number | null;
    ceil: number | null;
  };
}

const bstFloorCeilIterativeDefinition: AlgorithmDefinition<BstFloorCeilIterativeInput> = {
  meta: {
    id: ALGORITHM_ID.BST_FLOOR_CEIL_ITERATIVE!,
    name: "BST Floor & Ceil (Iterative)",
