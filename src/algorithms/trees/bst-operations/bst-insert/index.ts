...
import { bstInsertEducational } from "./educational";
...
function executeBstInsert(input: BstInsertInput): number {
  ...
  const result = bstInsert(root, input.insertValue) as BSTNodeShape;
  return result.value;
}

const bstInsertDefinition: AlgorithmDefinition<BstInsertInput> = {
  meta: {
    id: ALGORITHM_ID.BST_INSERT!,
    name: "BST Insert (Recursive)",
