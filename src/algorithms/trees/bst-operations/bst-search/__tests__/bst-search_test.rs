...
mod tests {
    ...
    #[test]
    fn test_finds_existing_value() {
        assert_eq!(bst_search(&build_tree(), 5), Some(5));
    }

    #[test]
    fn test_returns_none_for_missing() {
        assert_eq!(bst_search(&build_tree(), 9), None);
    }

    #[test]
    fn test_finds_root() {
        assert_eq!(bst_search(&build_tree(), 4), Some(4));
    }

    #[test]
    fn test_finds_leaf() {
        assert_eq!(bst_search(&build_tree(), 1), Some(1));
    }

    #[test]
    fn test_null_tree() {
        assert_eq!(bst_search(&None, 5), None);
    }
}
