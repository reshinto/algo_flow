include!("../sources/right-side-view-recursive.rs");
...
mod tests {
    ...
    fn test_null_returns_empty() {
        assert_eq!(right_side_view_recursive(&None), vec![] as Vec<i32>);
    }

    #[test]
    fn test_single_node() {
        assert_eq!(right_side_view_recursive(&leaf(1)), vec![1]);
    }
    ...
    fn test_7_node_bst() {
        ...
        assert_eq!(right_side_view_recursive(&root), vec![4, 6, 7]);
    }

    #[test]
    fn test_left_skewed_tree() {
        let root = make_node(1, make_node(2, leaf(3), None), None);
        assert_eq!(right_side_view_recursive(&root), vec![1, 2, 3]);
    }
}
