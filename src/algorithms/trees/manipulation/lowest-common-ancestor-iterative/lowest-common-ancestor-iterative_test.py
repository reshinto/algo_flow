...
module = importlib.import_module("lowest-common-ancestor-iterative")
BinaryNode = module.BinaryNode
lowest_common_ancestor_iterative = module.lowest_common_ancestor_iterative
...
def test_null_root_returns_none():
    assert lowest_common_ancestor_iterative(None, 1, 2) is None


def test_root_matches_one_target():
    root = make_node(4, make_node(2), make_node(6))
    result = lowest_common_ancestor_iterative(root, 4, 6)
    assert result.value == 4


def test_lca_is_node_2_for_targets_1_and_3():
    result = lowest_common_ancestor_iterative(build_7_node_tree(), 1, 3)
    assert result.value == 2


def test_lca_is_root_for_opposite_subtrees():
    result = lowest_common_ancestor_iterative(build_7_node_tree(), 3, 5)
    assert result.value == 4


def test_ancestor_of_other():
    root = make_node(4, make_node(2, make_node(1)))
    result = lowest_common_ancestor_iterative(root, 2, 1)
    assert result.value == 2
