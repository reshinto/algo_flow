...
#include "sources/LowestCommonAncestorIterative.cpp"
#include <cassert>
#include <iostream>
...
int main() {
    ...
    assert(lowestCommonAncestorIterative(nullptr, 1, 2) == nullptr);

    // test: LCA(1,3) = 2
    assert(lowestCommonAncestorIterative(buildLCAI7NodeTree(), 1, 3)->value == 2);

    // test: LCA(3,5) = 4 (root)
    assert(lowestCommonAncestorIterative(buildLCAI7NodeTree(), 3, 5)->value == 4);

    // test: ancestor of other
    BinaryNode* tree = makeLCAINode(4, makeLCAINode(2, makeLCAINode(1), nullptr), nullptr);
    assert(lowestCommonAncestorIterative(tree, 2, 1)->value == 2);
