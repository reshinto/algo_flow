include!("sources/lowest-common-ancestor-iterative.rs");
...
mod tests {
    ...
    fn test_null_root_returns_none() {
        assert!(lowest_common_ancestor_iterative(&None, 1, 2).is_none());
    }

    #[test]
    fn test_lca_node_2_for_1_and_3() {
        let result = lowest_common_ancestor_iterative(&build_7_node_tree(), 1, 3);
        assert_eq!(result.unwrap(), 2);
    }

    #[test]
    fn test_lca_root_for_opposite_subtrees() {
        let result = lowest_common_ancestor_iterative(&build_7_node_tree(), 3, 5);
    ...
    fn test_ancestor_of_other() {
        ...
        let result = lowest_common_ancestor_iterative(&root, 2, 1);
        assert_eq!(result.unwrap(), 2);
    }
}
