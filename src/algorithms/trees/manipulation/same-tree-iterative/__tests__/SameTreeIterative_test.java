// javac *.java && java -ea SameTreeIterative_test
public class SameTreeIterative_test {
    ...
    public static void main(String[] args) {
        SameTreeIterative algo = new SameTreeIterative();

        // test: two null trees
        assert algo.sameTreeIterative(null, null) == true : "Two nulls should be true";

        // test: one null tree
        assert algo.sameTreeIterative(leaf(1), null) == false : "One null should be false";

        // test: identical single nodes
        assert algo.sameTreeIterative(leaf(1), leaf(1)) == true : "Identical single nodes should be true";

        // test: different single nodes
        assert algo.sameTreeIterative(leaf(1), leaf(2)) == false : "Different single nodes should be false";
        ...
        assert algo.sameTreeIterative(treeA, treeB) == true : "Identical 7-node BSTs should be true";

        // test: different leaf values
        assert algo.sameTreeIterative(
