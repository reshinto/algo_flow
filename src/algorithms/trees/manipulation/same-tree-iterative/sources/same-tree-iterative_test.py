...
module = importlib.import_module("same-tree-iterative")
...
def test_two_null_trees():
    assert same_tree_iterative(None, None) == True


def test_one_null_tree():
    assert same_tree_iterative(make_node(1), None) == False
    assert same_tree_iterative(None, make_node(1)) == False


def test_identical_single_nodes():
    assert same_tree_iterative(make_node(1), make_node(1)) == True


def test_different_single_nodes():
    assert same_tree_iterative(make_node(1), make_node(2)) == False
