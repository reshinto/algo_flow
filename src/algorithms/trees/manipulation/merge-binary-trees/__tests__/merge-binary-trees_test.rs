include!("../sources/merge-binary-trees.rs");
...
mod tests {
    ...
    fn test_tree_a_null_returns_tree_b() {
        let tree_b = leaf(1);
        let result = merge_binary_trees(None, tree_b);
        assert_eq!(result.unwrap().value, 1);
    }

    #[test]
    fn test_sums_two_single_nodes() {
        let result = merge_binary_trees(leaf(3), leaf(5));
    ...
    fn test_merges_7_node_trees() {
        ...
        let result = merge_binary_trees(tree_a, tree_b);
