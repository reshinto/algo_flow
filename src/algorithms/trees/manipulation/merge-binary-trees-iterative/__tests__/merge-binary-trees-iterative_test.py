...
module = importlib.import_module("merge-binary-trees-iterative")
BinaryNode = module.BinaryNode
merge_binary_trees_iterative = module.merge_binary_trees_iterative
...
def test_tree_a_null_returns_tree_b():
    tree_b = make_node(1)
    assert merge_binary_trees_iterative(None, tree_b) is tree_b


def test_tree_b_null_returns_tree_a():
    tree_a = make_node(1)
    assert merge_binary_trees_iterative(tree_a, None) is tree_a


def test_sums_two_single_nodes():
    result = merge_binary_trees_iterative(make_node(3), make_node(5))
...
def test_merges_7_node_trees():
    ...
    result = merge_binary_trees_iterative(tree_a, tree_b)
