...
int main() {
    LowestCommonAncestor algo;

    // test: null root returns null
    assert(algo.lowestCommonAncestor(nullptr, 1, 2) == nullptr);

    // test: LCA(1,3) = 2
    assert(algo.lowestCommonAncestor(buildLCA7NodeTree(), 1, 3)->value == 2);

    // test: LCA(3,5) = 4 (root)
    assert(algo.lowestCommonAncestor(buildLCA7NodeTree(), 3, 5)->value == 4);

    // test: ancestor of other
    BinaryNode* tree = makeLCANode(4, makeLCANode(2, makeLCANode(1), nullptr), nullptr);
    assert(algo.lowestCommonAncestor(tree, 2, 1)->value == 2);
