...
describe("sameTree", () => {
  ...
  it("returns false when one tree is null", () => {
    expect(sameTree(makeNode(1), null)).toBe(false);
    expect(sameTree(null, makeNode(1))).toBe(false);
  });

  it("returns true for two identical single-node trees", () => {
    expect(sameTree(makeNode(1), makeNode(1))).toBe(true);
  });

  it("returns false for single-node trees with different values", () => {
    expect(sameTree(makeNode(1), makeNode(2))).toBe(false);
  });
  ...
  it("returns true for two identical 7-node BSTs", () => {
    ...
    expect(sameTree(treeA, treeB)).toBe(true);
  });
  ...
  it("returns false for trees with different leaf values", () => {
    ...
    expect(sameTree(treeA, treeB)).toBe(false);
  });
