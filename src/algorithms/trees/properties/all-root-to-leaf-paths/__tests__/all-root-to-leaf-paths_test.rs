include!("../sources/all-root-to-leaf-paths.rs");
...
mod tests {
    ...
    fn test_returns_4_paths_for_7_node_bst() {
        ...
        let paths = all_root_to_leaf_paths(&root);
        assert_eq!(paths.len(), 4);
    }

    #[test]
    fn test_contains_correct_paths() {
        let root = make_node(4,
            make_node(2, leaf(1), leaf(3)),
            make_node(6, leaf(5), leaf(7)));
        let paths = all_root_to_leaf_paths(&root);
        assert!(paths.contains(&"4->2->1".to_string()));
        assert!(paths.contains(&"4->2->3".to_string()));
    }

    #[test]
    fn test_empty_for_null_root() {
        assert_eq!(all_root_to_leaf_paths(&None), Vec::<String>::new());
    }

    #[test]
    fn test_single_node() {
        let paths = all_root_to_leaf_paths(&leaf(5));
