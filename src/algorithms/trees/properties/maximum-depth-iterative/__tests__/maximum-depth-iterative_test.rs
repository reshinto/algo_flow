include!("../sources/maximum-depth-iterative.rs");
...
mod tests {
    ...
    fn test_balanced_7_node_bst() {
        ...
        assert_eq!(maximum_depth_iterative(root), 3);
    }

    #[test]
    fn test_null_root() {
        assert_eq!(maximum_depth_iterative(None), 0);
    }

    #[test]
    fn test_single_node() {
        assert_eq!(maximum_depth_iterative(leaf(42)), 1);
    }

    #[test]
    fn test_left_skewed_tree() {
        let root = make_node(5, make_node(4, make_node(3, make_node(2, leaf(1), None), None), None), None);
        assert_eq!(maximum_depth_iterative(root), 5);
    }

    #[test]
    fn test_two_level_tree() {
        let root = make_node(1, leaf(2), None);
        assert_eq!(maximum_depth_iterative(root), 2);
    }
}
