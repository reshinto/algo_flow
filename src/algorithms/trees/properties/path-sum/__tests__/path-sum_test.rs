...
mod tests {
    ...
    fn test_path_sum_exists() {
        ...
        assert_eq!(path_sum(root, 7), true);
    }
    ...
    fn test_path_sum_not_exists() {
        let root = make_node(4,
            make_node(2, leaf(1), leaf(3)),
            make_node(6, leaf(5), leaf(7)));
        assert_eq!(path_sum(root, 100), false);
    }

    #[test]
    fn test_null_root() {
        assert_eq!(path_sum(None, 5), false);
    }

    #[test]
    fn test_single_node_matching() {
        assert_eq!(path_sum(leaf(5), 5), true);
    }

    #[test]
    fn test_single_node_not_matching() {
        assert_eq!(path_sum(leaf(5), 3), false);
    }
}
