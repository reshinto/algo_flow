...
import { pathSumIterativeEducational } from "./educational";
...
function executePathSumIterative(input: PathSumIterativeInput): boolean {
  interface BSTNode {
    value: number;
    left: BSTNode | null;
    right: BSTNode | null;
  ...
  return pathSumIterative(toBSTNode(input.rootId), input.targetSum) as boolean;
}

const pathSumIterativeDefinition: AlgorithmDefinition<PathSumIterativeInput> = {
  meta: {
    id: ALGORITHM_ID.PATH_SUM_ITERATIVE!,
    name: "Path Sum (Iterative)",
