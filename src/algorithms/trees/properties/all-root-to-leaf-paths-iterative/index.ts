...
import { generateAllRootToLeafPathsIterativeSteps } from "./step-generator";
import type { AllRootToLeafPathsIterativeInput } from "./step-generator";
import { allRootToLeafPathsIterativeEducational } from "./educational";
...
function executeAllRootToLeafPathsIterative(input: AllRootToLeafPathsIterativeInput): string[] {
  interface BSTNode {
    value: number;
    left: BSTNode | null;
    right: BSTNode | null;
  ...
  return allRootToLeafPathsIterative(toBSTNode(input.rootId)) as string[];
}

const allRootToLeafPathsIterativeDefinition: AlgorithmDefinition<AllRootToLeafPathsIterativeInput> =
  {
    meta: {
      id: ALGORITHM_ID.ALL_ROOT_TO_LEAF_PATHS_ITERATIVE!,
      name: "All Root to Leaf Paths (Iterative)",
