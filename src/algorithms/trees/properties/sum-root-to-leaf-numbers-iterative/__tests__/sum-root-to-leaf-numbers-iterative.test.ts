...
describe("sumRootToLeafNumbersIterative", () => {
  it("returns correct sum for a 7-node BST (421+423+465+467=1776)", () => {
    const root = node(4, node(2, node(1), node(3)), node(6, node(5), node(7)));
    expect(sumRootToLeafNumbersIterative(root)).toBe(1776);
  });

  it("returns 0 for null root", () => {
    expect(sumRootToLeafNumbersIterative(null)).toBe(0);
  });

  it("returns node value for single node", () => {
    expect(sumRootToLeafNumbersIterative(node(5))).toBe(5);
  });

  it("returns correct value for simple 3-node tree (12+13=25)", () => {
    expect(sumRootToLeafNumbersIterative(node(1, node(2), node(3)))).toBe(25);
  });
});
