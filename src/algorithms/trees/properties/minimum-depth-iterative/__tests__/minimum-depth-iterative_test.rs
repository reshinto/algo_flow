include!("../sources/minimum-depth-iterative.rs");
...
mod tests {
    ...
    fn test_balanced_7_node_bst() {
        ...
        assert_eq!(minimum_depth_iterative(root), 3);
    }

    #[test]
    fn test_null_root() {
        assert_eq!(minimum_depth_iterative(None), 0);
    }

    #[test]
    fn test_single_node() {
        assert_eq!(minimum_depth_iterative(leaf(42)), 1);
    }

    #[test]
    fn test_two_level_tree() {
        let root = make_node(1, leaf(2), None);
        assert_eq!(minimum_depth_iterative(root), 2);
    }
}
