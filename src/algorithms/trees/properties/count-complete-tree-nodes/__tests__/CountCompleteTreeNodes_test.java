...
public class CountCompleteTreeNodes_test {
    static CompleteTreeNode makeNode(int value, CompleteTreeNode left, CompleteTreeNode right) {
        CompleteTreeNode node = new CompleteTreeNode(value);
        node.left = left;
        node.right = right;
        return node;
    }

    static CompleteTreeNode leaf(int value) { return new CompleteTreeNode(value); }

    public static void main(String[] args) {
        CountCompleteTreeNodes algo = new CountCompleteTreeNodes();

        // test: 7-node perfect tree
        CompleteTreeNode tree1 = makeNode(4, makeNode(2, leaf(1), leaf(3)), makeNode(6, leaf(5), leaf(7)));
