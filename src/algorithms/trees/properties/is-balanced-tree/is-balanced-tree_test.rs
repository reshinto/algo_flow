include!("sources/is-balanced-tree.rs");
...
mod tests {
    ...
    fn test_balanced_7_node_bst() {
        ...
        assert_eq!(is_balanced_tree(root), true);
    }

    #[test]
    fn test_null_root() {
        assert_eq!(is_balanced_tree(None), true);
    }

    #[test]
    fn test_single_node() {
        assert_eq!(is_balanced_tree(leaf(1)), true);
    }

    #[test]
    fn test_unbalanced_tree() {
        let root = make_node(1, make_node(2, make_node(3, leaf(4), None), None), None);
        assert_eq!(is_balanced_tree(root), false);
    }
