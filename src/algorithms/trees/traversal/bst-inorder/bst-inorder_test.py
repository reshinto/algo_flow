...
mod = importlib.import_module("bst-inorder")
bst_inorder = mod.bst_inorder
BSTNode = mod.BSTNode
...
def test_balanced_7_node_bst():
    ...
    assert bst_inorder(root) == [1, 2, 3, 4, 5, 6, 7]


def test_null_root():
    assert bst_inorder(None) == []


def test_single_node():
    assert bst_inorder(make_node(42)) == [42]


def test_left_skewed():
    root = make_node(5, make_node(4, make_node(3, make_node(2, make_node(1)))))
    assert bst_inorder(root) == [1, 2, 3, 4, 5]
...
def test_left_child_only():
    assert bst_inorder(make_node(5, make_node(3))) == [3, 5]


def test_right_child_only():
    assert bst_inorder(make_node(5, None, make_node(8))) == [5, 8]
