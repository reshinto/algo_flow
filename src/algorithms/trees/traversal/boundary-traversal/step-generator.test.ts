...
describe("generateBoundaryTraversalSteps", () => {
  ...
  it("visits all 7 unique boundary nodes", () => {
    const steps = generateBoundaryTraversalSteps({ nodes: defaultNodes, rootId: "n4" });
    const visitSteps = steps.filter((step) => step.type === "visit");
    const uniqueValues = new Set(visitSteps.map((step) => step.variables["value"] as number));
