...
describe("bstInorderIterative", () => {
  ...
  it("handles a two-node tree with only a left child", () => {
    expect(bstInorderIterative(node(5, node(3)))).toEqual([3, 5]);
  });

  it("handles a two-node tree with only a right child", () => {
    expect(bstInorderIterative(node(5, null, node(8)))).toEqual([5, 8]);
  });
});
