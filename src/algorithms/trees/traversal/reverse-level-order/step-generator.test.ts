...
describe("generateReverseLevelOrderSteps", () => {
  ...
  it("visits leaf nodes before root (bottom-up order)", () => {
    const steps = generateReverseLevelOrderSteps({ nodes: defaultNodes, rootId: "n4" });
    const visitSteps = steps.filter((step) => step.type === "visit");
    const visitedValues = visitSteps.map((step) => step.variables["value"] as number);
    // Bottom level first: 1, 3, 5, 7, then level 1: 2, 6, then root: 4
    expect(visitedValues).toEqual([1, 3, 5, 7, 2, 6, 4]);
  });
