include!("../sources/morris-inorder-traversal.rs");
...
mod tests {
    ...
    fn test_balanced_7_node_bst() {
        ...
        assert_eq!(morris_inorder_traversal(root), vec![1, 2, 3, 4, 5, 6, 7]);
    }

    #[test]
    fn test_null_root() {
        assert_eq!(morris_inorder_traversal(None), Vec::<i32>::new());
    }

    #[test]
    fn test_single_node() {
        assert_eq!(morris_inorder_traversal(leaf(42)), vec![42]);
    }

    #[test]
    fn test_left_skewed() {
        let root = make_node(5, make_node(4, make_node(3, make_node(2, leaf(1), None), None), None), None);
        assert_eq!(morris_inorder_traversal(root), vec![1, 2, 3, 4, 5]);
    }
