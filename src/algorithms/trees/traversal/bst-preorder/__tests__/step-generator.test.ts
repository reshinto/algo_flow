...
describe("generateBstPreorderSteps", () => {
  ...
  it("visits root node (value 4) first", () => {
    const steps = generateBstPreorderSteps({ nodes: defaultNodes, rootId: "n4" });
    const visitSteps = steps.filter((step) => step.type === "visit");
    expect(visitSteps[0]?.variables["value"]).toBe(4);
  });

  it("visits nodes in pre-order (NLR) sequence", () => {
    const steps = generateBstPreorderSteps({ nodes: defaultNodes, rootId: "n4" });
    const visitSteps = steps.filter((step) => step.type === "visit");
    const visitedValues = visitSteps.map((step) => step.variables["value"] as number);
    expect(visitedValues).toEqual([4, 2, 1, 3, 6, 5, 7]);
  });
