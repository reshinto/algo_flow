include!("build-from-preorder-inorder-iterative.rs");
...
mod tests {
    ...
    fn test_returns_none_for_empty() {
        assert!(build_from_preorder_inorder_iterative(vec![], vec![]).is_none());
    }

    #[test]
    fn test_single_node() {
        let root = build_from_preorder_inorder_iterative(vec![42], vec![42]);
