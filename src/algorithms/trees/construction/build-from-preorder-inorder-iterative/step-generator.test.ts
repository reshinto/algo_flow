...
describe("generateBuildFromPreorderInorderIterativeSteps", () => {
  it("produces steps for a 7-node tree", () => {
    const steps = generateBuildFromPreorderInorderIterativeSteps(defaultInput);
  ...
  it("handles a single-element input", () => {
    const steps = generateBuildFromPreorderInorderIterativeSteps({ preorder: [5], inorder: [5] });
    expect(steps[steps.length - 1]?.type).toBe("complete");
  });
});
