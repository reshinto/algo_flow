include!("../sources/build-from-postorder-inorder-iterative.rs");
...
mod tests {
    ...
    fn test_returns_none_for_empty() {
        assert!(build_from_postorder_inorder_iterative(&[], &[]).is_none());
    }

    #[test]
    fn test_single_node() {
        let root = build_from_postorder_inorder_iterative(&[7], &[7]);
