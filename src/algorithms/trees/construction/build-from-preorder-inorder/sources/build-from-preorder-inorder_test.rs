include!("build-from-preorder-inorder.rs");
...
mod tests {
    ...
    fn test_returns_none_for_empty() {
        assert!(build_from_preorder_inorder(vec![], vec![]).is_none());
    }

    #[test]
    fn test_single_node() {
        let root = build_from_preorder_inorder(vec![42], vec![42]);
        assert_eq!(root.as_ref().unwrap().value, 42);
    }

    #[test]
    fn test_right_skewed_tree() {
        let root = build_from_preorder_inorder(vec![1, 2, 3], vec![1, 2, 3]);
