...
module = importlib.import_module("build-from-preorder-inorder")
TreeNode = module.TreeNode
build_from_preorder_inorder = module.build_from_preorder_inorder
...
def test_builds_balanced_7_node_bst():
    root = build_from_preorder_inorder([4, 2, 1, 3, 6, 5, 7], [1, 2, 3, 4, 5, 6, 7])
    assert root.value == 4
...
def test_preserves_inorder():
    ...
    root = build_from_preorder_inorder([4, 2, 1, 3, 6, 5, 7], input_inorder)
    assert inorder(root) == input_inorder


def test_preserves_preorder():
    input_preorder = [4, 2, 1, 3, 6, 5, 7]
    root = build_from_preorder_inorder(input_preorder, [1, 2, 3, 4, 5, 6, 7])
...
def test_single_node():
    root = build_from_preorder_inorder([42], [42])
