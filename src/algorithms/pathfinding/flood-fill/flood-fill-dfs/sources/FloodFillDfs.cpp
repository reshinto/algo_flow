...
FloodFillResult floodFillDfs(const std::vector<std::vector<GridCell>>& grid,
  ...
  std::stack<std::pair<int, int>> dfsStack; // @step:initialize,open-node
  ...
  while (!dfsStack.empty()) {
    ...
    auto current = dfsStack.top(); // @step:close-node
    ...
    for (int dirIndex = 0; dirIndex < 4; dirIndex++) {
      ...
      dfsStack.push({neighborRow, neighborCol}); // @step:open-node
    }
  }
