/**
 * Storybook stories for the Flood Fill BFS pipeline.
...
function buildStoryGrid(): GridCell[][] {
  ...
  /* Add an L-shaped wall barrier */
  for (let wallRow = 1; wallRow <= 5; wallRow++) {
    const cell = grid[wallRow]?.[5];
    if (cell) cell.type = "wall";
  }
  for (let wallCol = 5; wallCol <= 9; wallCol++) {
    const cell = grid[5]?.[wallCol];
    if (cell) cell.type = "wall";
  }
...
const meta: Meta<typeof GridVisualizer> = {
  title: "Algorithm Pipelines/FloodFillBfs",
...
/** Mid-fill with open and closed cells spreading from start */
...
/** Fill complete — all reachable cells closed/filled */
