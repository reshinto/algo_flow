...
flood_fill_bfs_mod = importlib.import_module("flood-fill-bfs")
flood_fill_bfs = flood_fill_bfs_mod.flood_fill_bfs
...
def test_fills_all_cells_on_small_empty_grid():
    ...
    result = flood_fill_bfs(grid, (0, 0))
...
def test_enclosed_region():
    grid = make_empty_grid(5, 5)
    for col in range(5):
        set_cell(grid, 0, col, "wall")
        set_cell(grid, 4, col, "wall")
    for row in range(1, 4):
        set_cell(grid, row, 0, "wall")
        set_cell(grid, row, 4, "wall")
    result = flood_fill_bfs(grid, (2, 2))
    assert result["count"] == 9


def test_seed_cell_is_first_filled():
    grid = make_empty_grid(3, 3)
    result = flood_fill_bfs(grid, (1, 1))
