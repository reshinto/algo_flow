...
import { floodFillBfsEducational } from "./educational";
...
const floodFillBfsDefinition: AlgorithmDefinition<FloodFillBfsInput> = {
  meta: {
    id: ALGORITHM_ID.FLOOD_FILL_BFS!,
    name: "Flood Fill BFS",
    category: CATEGORY.PATHFINDING!,
    technique: "flood-fill",
    description:
      "Classic paint-bucket flood fill using BFS — fills all connected empty cells reachable from the start position in wave-like order",
    timeComplexity: {
      best: "O(1)",
      average: "O(V + E)",
      worst: "O(V + E)",
  ...
  execute: (input: FloodFillBfsInput) => floodFillBfs(input.grid, input.startPosition),
  generateSteps: generateFloodFillBfsSteps,
  educational: floodFillBfsEducational,
  sources: {
...
registry.register(floodFillBfsDefinition);
