...
describe("generatePrimsMazeSteps", () => {
  ...
  it("includes open-node steps for frontier tracking", () => {
    const grid = createAllWallsGrid(7, 7);
    setCell(grid, 1, 1, "start");
    setCell(grid, 5, 5, "end");
    ...
    const openSteps = steps.filter((step) => step.type === "open-node");
    expect(openSteps.length).toBeGreaterThan(0);
  });
