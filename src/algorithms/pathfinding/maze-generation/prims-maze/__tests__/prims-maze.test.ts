...
describe("primsMaze", () => {
  it("carves passages from an all-walls grid", () => {
    ...
    const result = primsMaze(grid, [1, 1]);
  ...
  it("carves more passages on a larger grid", () => {
    const smallGrid = createAllWallsGrid(7, 7);
    setCell(smallGrid, 1, 1, "start");
    setCell(smallGrid, 5, 5, "end");
    const smallResult = primsMaze(smallGrid, [1, 1]);

    const largeGrid = createAllWallsGrid(11, 11);
    setCell(largeGrid, 1, 1, "start");
    setCell(largeGrid, 9, 9, "end");
    const largeResult = primsMaze(largeGrid, [1, 1]);

    expect(largeResult.passagesCarved).toBeGreaterThan(smallResult.passagesCarved);
  });

  it("start cell is carved (empty or start type)", () => {
    const grid = createAllWallsGrid(9, 9);
    setCell(grid, 1, 1, "start");
    setCell(grid, 7, 7, "end");
