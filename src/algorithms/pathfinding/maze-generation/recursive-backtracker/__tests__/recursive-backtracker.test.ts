...
describe("recursiveBacktrackerMaze", () => {
  it("carves passages in an all-walls grid", () => {
    const grid = createAllWallsGrid(9, 9);
    setCell(grid, 1, 1, "start");
    setCell(grid, 7, 7, "end");

    const result = recursiveBacktrackerMaze(grid, [1, 1]);

    expect(result.passagesCarved).toBeGreaterThan(0);
  });

  it("reduces wall count after generation", () => {
    const grid = createAllWallsGrid(9, 9);
    setCell(grid, 1, 1, "start");
    setCell(grid, 7, 7, "end");
  ...
  it("creates a connected maze where start can reach end", () => {
    ...
    const reachable = bfsReachable(grid, [1, 1], [7, 7]);
    expect(reachable).toBe(true);
  });

  it("returns passagesCarved count matching actual empty cells created", () => {
  ...
  it("does not carve out-of-bounds or border cells", () => {
    const grid = createAllWallsGrid(9, 9);
    setCell(grid, 1, 1, "start");
    setCell(grid, 7, 7, "end");
    ...
    // Border cells (row 0, row 8, col 0, col 8) should remain walls
