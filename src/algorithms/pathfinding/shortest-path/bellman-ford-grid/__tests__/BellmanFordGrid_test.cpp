#include "../sources/BellmanFordGrid.cpp"
#include <cassert>
#include <iostream>
...
int main() {
  // Test: finds path
  {
    ...
    auto result = bellmanFordGrid(grid, {0, 0}, {4, 4});
  ...
  // Test: path empty when blocked
  {
    ...
    auto result = bellmanFordGrid(grid, {0, 0}, {2, 2});
    assert(result.path.empty());
  }
  ...
  // Test: adjacent cells
  {
    ...
    auto result = bellmanFordGrid(grid, {0, 0}, {0, 1});
