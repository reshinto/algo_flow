...
describe("bfsShortestPath", () => {
  it("finds a direct path on an empty grid", () => {
    ...
    const result = bfsShortestPath(grid, [0, 0], [4, 4]);
  ...
  it("navigates around walls", () => {
    ...
    const result = bfsShortestPath(grid, [0, 0], [0, 4]);
  ...
  it("handles adjacent start and end", () => {
    ...
    const result = bfsShortestPath(grid, [0, 0], [0, 1]);
  ...
  it("handles start equal to end", () => {
    ...
    const result = bfsShortestPath(grid, [1, 1], [1, 1]);
  ...
  it("tracks visited cells", () => {
    ...
    const result = bfsShortestPath(grid, [0, 0], [2, 2]);

    expect(result.visited.length).toBeGreaterThan(0);
    expect(result.visited[0]).toEqual([0, 0]);
  });
});
