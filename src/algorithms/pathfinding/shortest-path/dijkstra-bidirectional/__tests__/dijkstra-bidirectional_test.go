...
func TestFindsPath(t *testing.T) {
	...
	result := DijkstraBidirectional(grid, [2]int{0, 0}, [2]int{4, 4})
...
func TestPathEmptyWhenBlocked(t *testing.T) {
	...
	result := DijkstraBidirectional(grid, [2]int{0, 0}, [2]int{2, 2})
	if len(result.Path) != 0 {
...
func TestAdjacentCells(t *testing.T) {
	...
	result := DijkstraBidirectional(grid, [2]int{0, 0}, [2]int{0, 1})
