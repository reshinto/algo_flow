#include "sources/GreedyBestFirst.cpp"
#include <cassert>
#include <iostream>
...
int main() {
  // Test: finds path on empty grid
  {
    ...
    auto result = greedyBestFirst(grid, {0, 0}, {4, 4});
  ...
  // Test: handles adjacent start and end
  {
    ...
    auto result = greedyBestFirst(grid, {0, 0}, {0, 1});
  ...
  {
    auto grid = makeEmptyGrid(3, 3);
    auto result = greedyBestFirst(grid, {1, 1}, {1, 1});
  ...
  {
    auto grid = makeEmptyGrid(3, 3);
    auto result = greedyBestFirst(grid, {0, 0}, {2, 2});
