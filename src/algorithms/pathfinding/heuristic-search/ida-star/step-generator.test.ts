...
describe("generateIDAStarSteps", () => {
  ...
  it("shows DFS-style close-node steps", () => {
    const grid = createEmptyGrid(3, 3);
    const steps = generateIDAStarSteps({
      grid,
      startPosition: [0, 0],
    ...
    const closeSteps = steps.filter((step) => step.type === "close-node");
    expect(closeSteps.length).toBeGreaterThan(0);
  });
});
