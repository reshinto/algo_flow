...
vector<vector<int>> pascalsTriangle(int numRows) {
    vector<vector<int>> triangle; // @step:initialize

    for (int rowIdx = 0; rowIdx < numRows; rowIdx++) {
        // @step:initialize
        vector<int> row(rowIdx + 1, 0); // @step:initialize

        row[0] = 1; // @step:compute-value
        row[rowIdx] = 1; // @step:compute-value

        for (int colIdx = 1; colIdx < rowIdx; colIdx++) {
            const vector<int>& above = triangle[rowIdx - 1];
            row[colIdx] = above[colIdx - 1] + above[colIdx]; // @step:compute-value
        }

        triangle.push_back(row); // @step:complete
    }
