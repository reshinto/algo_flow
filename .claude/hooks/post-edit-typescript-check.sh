...
# Check 4: number[][] instead of tuple types
NESTED_ARRAYS=$(grep -nE 'number\[\]\[\]' "$FILE" || true)
if [ -n "$NESTED_ARRAYS" ]; then
  echo "WARN: [TypeScript] number[][] found — use tuple type [number, number][] for coordinates:" >&2
  echo "$NESTED_ARRAYS" | head -5 >&2
  WARNINGS=$((WARNINGS + 1))
fi

if [ "$WARNINGS" -gt 0 ]; then
  echo "TypeScript check: ${WARNINGS} warning(s) — see strict-typescript-review skill for details" >&2
fi
