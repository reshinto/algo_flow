#!/usr/bin/env bash
# PostToolUse hook: warn on accessibility anti-patterns in edited .tsx files.
...
if [ -n "$HEX_HITS" ]; then
  echo "WARN: [A11y] Raw hex colors found — use CSS custom properties from design tokens:" >&2
  echo "$HEX_HITS" | head -3 >&2
  WARNINGS=$((WARNINGS + 1))
fi

# Check 2: Interactive elements without aria-label
BUTTONS=$(grep -nE '<button|<input|<select' "$FILE" | grep -v 'aria-label' | grep -v 'aria-labelledby' | grep -v '^\s*//' || true)
if [ -n "$BUTTONS" ]; then
  echo "WARN: [A11y] Interactive elements without aria-label:" >&2
  echo "$BUTTONS" | head -3 >&2
  WARNINGS=$((WARNINGS + 1))
fi

# Check 3: outline: none without accessible replacement
...
# Check 4: Framer Motion without reduced-motion support
...
if [ "$WARNINGS" -gt 0 ]; then
  echo "Accessibility check: ${WARNINGS} warning(s) — see accessibility-audit skill for details" >&2
fi
