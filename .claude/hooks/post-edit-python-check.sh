#!/usr/bin/env bash
# PostToolUse hook: warn on Python anti-patterns in algorithm source files.
...
if [[ "$FILE" =~ /sources/ ]] && ! grep -q '@step:' "$FILE"; then
  echo "WARN: [Python] Algorithm source file missing @step: annotations" >&2
  WARNINGS=$((WARNINGS + 1))
fi

if [ "$WARNINGS" -gt 0 ]; then
  echo "Python check: ${WARNINGS} warning(s)" >&2
fi
exit 0
